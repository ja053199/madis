...
from models.MaDis_Stereo import MaDis_finetuning
...
def get_args_parser():
    parser = argparse.ArgumentParser('Test MaDis models on stereo matching task', add_help=False)
    # important argument
...
def _load_model_and_criterion(model_path, do_load_metrics, device):
    ...
    model = MaDis_finetuning(head, **ckpt_args.croco_args)
    ...
    for i in range(len(pairnames)):
        ...
        if 'submission' in save:
            ...
            predi_np = predi.permute(1, 2, 0).squeeze(
                2).cpu().numpy()  # transform into HxWx2 for flow or HxW for stereo
...
def main(args):
    ...
    for i, dataloader in enumerate(dataloaders):
        ...
        for data_iter_step, (image1, image2, gt, pairnames) in enumerate(tqdm(dataloader)):
            ...
            do_flip = (task == 'stereo' and dstr.startswith('Spring') and any("right" in p for p in pairnames))
            # flip the images and will flip the prediction after as we assume img1 is on the left
            ...
            if do_flip:
                ...
                image1 = image1.flip(dims=[3])  # this is already the right frame, so flip it
                image2 = image2.flip(dims=[3])
                gt = gt

            with torch.inference_mode():
                pred, _, _, time = tiled_pred(model, None, None, image1, image2, None if dataset.name == 'Spring' else gt,
