...
class DPTOutputAdapter(nn.Module):
    ...
    def check(self, encoder_tokens, nh, nw):
        # append masked tokens to the sequence
        B, Nenc, C = encoder_tokens.size()

        if Nenc == nh * nw:
            return encoder_tokens.to("cuda")
        else:
            Ntotal = nh * nw
            func = nn.Parameter(torch.zeros(1, 1, 768)) #768
            f1_ = func.repeat(B, Ntotal, 1).to(dtype=encoder_tokens.dtype)
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], image_size):
        ...
        layers = [self.check(l, N_H, N_W) for l in layers]
