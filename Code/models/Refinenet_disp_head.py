...
from .fusion_blocks import DPTOutputAdapter
...
class PixelwiseTaskWithDPT(nn.Module):
    ...
    * for encoder+decoder: last encoder + 3 equally spread layers of the decoder
    ...
    def setup(self, croconet):
        ...
        if self.hooks_idx is None:
            if hasattr(croconet, 'dec_blocks'):  # encoder + decoder
                step = {8: 2, 12: 4, 24: 8}[croconet.dec_depth]
    ...
    def forward(self, x, img_info, mask=None):
        if mask is not None:
            out = self.dpt(x, image_size=(img_info['height'], img_info['width']), mask=mask)
