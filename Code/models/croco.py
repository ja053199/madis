...
from models.img_masking import RandomMask

class CroCoNet(nn.Module):

    def __init__(self,
                 img_size=(352, 1216),  # img size (KITTI dataset)
                 mask_ratio=0.90,  # ratios of masked tokens
    ...
    def _add_mask_token(self, enc_embed_dim):
        self.mask_token_downstream = nn.Parameter(torch.zeros(1, 1, enc_embed_dim))
    ...
    def forward(self, img1, img2, mode=None):
        if not self.update_decoder: # ema update occur on encoder
            if mode == 1:  # non-masking
                feat1, pos1, _ = self._encode_image(img1, do_mask=False, return_all_blocks=True)
                feat2, pos2, _ = self._encode_image(img2, do_mask=False, return_all_blocks=False)
                return feat1, pos1, feat2, pos2
            if mode == 2: # only right img masking
                feat2, pos2, mask2 = self._encode_image(img2, do_mask=True)
                return feat2, pos2, mask2
