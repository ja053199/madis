...
from models.MaDis_Stereo import MaDis_finetuning, croco_args_from_ckpt
from models.pos_embed import interpolate_pos_embed
from models.Refinenet_disp_head import PixelwiseTaskWithDPT
...
def get_args_parser():
    # prepare subparsers
    parser = argparse.ArgumentParser('Finetuning MaDis models on stereo matching', add_help=False)
    ...
    add_arg('--mask_ratio', default=0.4, type=float, help='masking ratio for input image')
...
def main(args):
    ...
    criterion3 = LaplacianLossBounded3() # depth loss that considering pgt
    ...
    croco_args['mask_ratio'] = args.mask_ratio
    print('Croco args: ' + str(croco_args))
    args.croco_args = croco_args  # save for test time
    # prepare head
    ...
    encoder = CroCoNet(**croco_args, update_decoder=False)
    model = MaDis_finetuning(head, **croco_args)
    interpolate_pos_embed(model, ckpt['model'])
    enc = encoder.load_state_dict(ckpt['model'], strict=False)
    ...
    if not args.resume and args.start_from:
        ...
        best_so_far = misc.load_model(args=args, encoder=encoder, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, encoder, criterion, criterion2, criterion3, metrics, data_loader_train,
        ...
        if data_loaders_val is not None and args.eval_every > 0 and (epoch + 1) % args.eval_every == 0:
            ...
            val_stats = validate_one_epoch(model, encoder, criterion, metrics, data_loaders_val, device, epoch,
            ...
            if val_best <= best_so_far:
                ...
                misc.save_model(args=args, encoder=encoder, model_without_ddp=model_without_ddp, optimizer=optimizer,
