...
def ema_update(student, teacher):
    alpha = 0.9999
    for (student_name, student_param), (teacher_name, teacher_param) in zip(student.named_parameters(), teacher.named_parameters()):
        if student_name.split('.')[0] == teacher_name.split('.')[0]:
            with torch.no_grad():
                update_params = alpha * teacher_param.data + (1 - alpha) * student_param.data
                teacher_param.data.copy_(update_params)

def train_one_epoch(model: torch.nn.Module, encoder: torch.nn.Module, criterion: torch.nn.Module,
                    criterion2: torch.nn.Module, criterion3: torch.nn.Module, metrics: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler,
                    log_writer=None, print_freq=20, args=None):
    model.train(True)
    ...
    for data_iter_step, (image1, image2, gt, pairname) in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=bool(args.amp)):
            visfeatL, visposL, visfeatR, visposR = encoder(img1=image1, img2=image2, mode=1) # non-masking
            prediction, reconL, maskL, targetL, reconR, maskR, targetR = model(img1=image1, img2=image2, visfeatL=visfeatL, visposL=visposL, visfeatR=visfeatR, visposR=visposR, mode=1)
            pgtDisp = model(img1=image1, img2=image2, visfeatL=visfeatL, visposL=visposL, visfeatR=visfeatR, visposR=visposR, mode=2)
            prediction, conf = split_prediction_conf(prediction, criterion.with_conf)
            pgtDisp, conf2 = split_prediction_conf(pgtDisp, criterion.with_conf)
            ...
            distLoss = criterion3(prediction, gt, pgtDisp, conf) if conf is not None else criterion3(prediction, gt, pgtDisp)
            reconLLoss = criterion2(reconL, maskL, targetL)
            reconRLoss = criterion2(reconR, maskR, targetR)
            loss = depthLoss + reconLLoss + reconRLoss + distLoss

        loss_value = loss.item()
        depthLoss_value = depthLoss.item()
        distLoss_value = distLoss.item()
        reconLLoss_value = reconLLoss.item()
        reconRLoss_value = reconRLoss.item()
        ...
        metric_logger.update(reconLLoss=reconLLoss_value)
        metric_logger.update(reconRLoss=reconRLoss_value)
        metric_logger.update(distLoss=distLoss_value)
    ...
    for didx, data_loader in enumerate(data_loaders):
        ...
        for data_iter_step, (image1, image2, gt, pairname) in enumerate(
            ...
            with torch.inference_mode():
                prediction, tiled_loss, c = tiled_pred(model, encoder, criterion, image1, image2, gt, conf_mode=conf_mode,
...
@torch.no_grad()
def tiled_pred(model, encoder, criterion, img1, img2, gt,
    ...
    for sy1, sx1, sy2, sx2, aligned in crop_generator():
        ...
        image1 = _crop(img1, sy1, sx1)
        image2 = _crop(img2, sy2, sx2)

        pred = model(img1=image1, img2=image2)
