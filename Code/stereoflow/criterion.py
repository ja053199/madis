...
class LaplacianLoss(nn.Module):  # used for CroCo-Stereo on ETH3D, d'=exp(d)
    ...
    def forward(self, predictions, gt, conf):
        ...
        conf = conf.squeeze(1)
        return (torch.abs(gt - predictions).sum(dim=1)[mask] / torch.exp(conf[mask]) + conf[
            mask]).mean()  # + torch.log(2) => which is a constant


class LaplacianLossBounded(nn.Module):
    ...
    def forward(self, predictions, gt, conf):
        ...
        return (torch.abs(gt - predictions).sum(dim=1)[mask] / conf[mask] + torch.log(conf)[
            mask]).mean()  # + torch.log(2) => which is a constant


class LaplacianLossBounded2(nn.Module):  # used for depth loss for MaDis-Stereo;
...
# Considering PGT
# if GT label exist, use GT loss
# if GT label is not exist, use PGT loss
class LaplacianLossBounded3(nn.Module): # used for dist loss for MaDis-Stereo;
    def __init__(self, max_gtnorm=None, a=3.0, b=3.0):
        super().__init__()
        self.max_gtnorm = max_gtnorm
        self.with_conf = True
    ...
    def forward(self, predictions, gt, pgt, conf):
        mask_gt = torch.isfinite(gt)
        mask_gt = mask_gt[:, 0, :, :]
        mask_pgt = ~mask_gt

        if self.max_gtnorm is not None:
            mask_gt *= _get_gtnorm(gt)[:, 0, :, :] < self.max_gtnorm
            mask_pgt *= _get_gtnorm(pgt)[:, 0, :, :] < self.max_gtnorm

        conf = conf.squeeze(1) if conf is not None else None
        conf = 2 * self.a * (torch.sigmoid(conf / self.b) - 0.5) if conf is not None else None

        loss_gt = (torch.abs(gt - predictions).sum(dim=1)[mask_gt] / torch.exp(conf[mask_gt]) + conf[
            mask_gt]).mean() if conf is not None else 0
        loss_pgt = (torch.abs(pgt - predictions).sum(dim=1)[mask_pgt] / torch.exp(conf[mask_pgt]) + conf[
            mask_pgt]).mean() if conf is not None else 0

        return loss_gt + loss_pgt
...
class StereoMetrics(nn.Module):
    ...
    def forward(self, predictions, gt):
        ...
        gtcopy[
            ~mask] = 999999.0  # we make a copy and put a non-infinite value, such that it does not become nan once multiplied by the mask value 0
        ...
        for ths in self.bad_ths:
            metrics['bad@{:.1f}'.format(ths)] = (((L1error > ths) * mask.view(B, -1)).sum(dim=1) / Npx).mean(
                dim=0) * 100
        return metrics
...
class FlowMetrics(nn.Module):
    ...
    def forward(self, predictions, gt):
        ...
        for ths in self.bad_ths:
            metrics['bad@{:.1f}'.format(ths)] = (((L2error > ths) * mask.view(B, -1)).sum(dim=1) / Npx).mean(
                dim=0) * 100
        return metrics
...
class StereoDatasetMetrics(nn.Module):
    ...
    def add_batch(self, predictions, gt):
        ...
        if gt.size(2) == predictions.size(2) * 2 and gt.size(3) == predictions.size(
                3) * 2:  # special case for Spring ...
...
class FlowDatasetMetrics(nn.Module):
    ...
    def add_batch(self, predictions, gt):
        ...
        if gt.size(2) == predictions.size(2) * 2 and gt.size(3) == predictions.size(
            ...
            valid = torch.isfinite(L1err)
            gtspeed = (torch.sqrt(torch.sum(torch.square(gt[:, :, 0::2, 0::2]), dim=1)) + torch.sqrt(
                torch.sum(torch.square(gt[:, :, 0::2, 1::2]), dim=1)) + \
                       torch.sqrt(torch.sum(torch.square(gt[:, :, 1::2, 0::2]), dim=1)) + torch.sqrt(
                        torch.sum(torch.square(gt[:, :, 1::2, 1::2]), dim=1))) / 4.0  # let's just average them
        else:
            valid = torch.isfinite(gt[:, 0, :, :])  # both x and y would be infinite
            L1err = torch.sum(torch.abs(gt - predictions), dim=1)
        ...
        for i, (th1, th2) in enumerate(self.speed_ths):
            ...
            self.agg_EPEspeed[i] = float(self.agg_Nspeed[i]) / iNnew * self.agg_EPEspeed[i] + float(iNspeed) / iNnew * \
                                   L2err[valid][vv].mean().cpu()
